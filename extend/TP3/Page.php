...
class Page {
    ...
    public function __construct($totalRows, $listRows = 20, $parameter = array()) {
        ...
        $skips = array('thinkphp_show_page_trace', 'PHPSESSID', 'sessid', '__forward__');
        foreach ($skips as $k) {
            if (isset($parameter[$k])) {
                unset($parameter[$k]);
            }
        }
        /* 基础设置 */
        ...
        $this->nowPage = max(1, I($this->p, 0, 'intval')? : 1);
    ...
    function show() {
        ...
        if ($this->clientmethod) {
            $this->url = 'javascript:' . $this->clientmethod . '(' . urlencode('[PAGE]') . ');';
        } else {
